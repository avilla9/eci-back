...
@section('subhead')
<title>Actualizar contenido - Conocimiento</title>
@endsection

@section('subcontent')
<div id="alert" class="hidden"></div>
<div class="intro-y flex flex-col sm:flex-row items-center mt-8">
  <h2 class="text-lg font-medium mr-auto">Actualizar contenido para la sección Conocimiento</h2>
  <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
    <button id="update-knowledge" class="btn btn-primary shadow-md flex items-center" aria-expanded="false">
...
<div id="form-body" class="pos intro-y grid grid-cols-12 gap-5 mt-5">
  ...
  <div class="intro-y col-span-12 lg:col-span-9">
    <div class="form-inline mt-3 mb-2">
      ...
      <select name="section" data-placeholder="Seleccione una sección" class="section-knowledge-select form-control tom-select w-full mb-2">
...
<script>
       document.querySelector('.section-knowledge-select').tomselect.addItem('{{ $article->section_id }}');
    ...
    let id = "{{ $article->id }}";
      if(unrestricted == 1) {
        ...
        $.ajax({
          ...
          url: "/api/posts/knowledge/filters/" + id,
   ...
   $('#update-knowledge').on('click', function (e) {
    e.preventDefault();
...
if (error) {
  ...
  $.ajax({
    type: "PUT",
    url: '/api/posts/knowledge/update',
    data: data,
    dataType: "JSON",
